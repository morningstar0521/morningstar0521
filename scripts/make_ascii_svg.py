...
portrait prints once and freezes.
    ...
    STATIC=1 python scripts/make_ascii_svg.py     # frozen state, for previews
    PREVIEW=1 python scripts/make_ascii_svg.py    # also dump plain text to stdout
"""
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "shubh-ascii.svg")
...
CONTRAST = 1.10
BRIGHTNESS = 1.0
GAMMA = 1.30          # <1 lightens mids; >1 deepens them. tuned so the face
                      # lands mid-ramp and the hair doesn't flatten into a blob
SHARPEN = True
...
PROMPT = "shubh@github: ~$ ./portrait.sh"
WHOAMI_NAME = "Shubh Ghiya"
WHOAMI_LABEL = "shubh@github:~$ whoami "
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">{html.escape(PROMPT)}</text>')
...
cursor_x = PAD + int(len(WHOAMI_LABEL + WHOAMI_NAME) * 7.82)
parts.append(f'<line x1="0" y1="{status_line_y:.1f}" x2="{CANVAS_W}" y2="{status_line_y:.1f}" stroke="{FRAME}"/>')
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{html.escape(WHOAMI_LABEL)}<tspan fill="{INK}">{html.escape(WHOAMI_NAME)}</tspan></text>')
