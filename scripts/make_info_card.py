...
colored key/value rows for work, tech stack, and highlights - NOT GitHub stats
...
HOST_USER = "shubh"
HOST_NAME = "github"
PROMPT = f"{HOST_USER}@{HOST_NAME}: ~$ neofetch"

# content model: tuples describing each row
# ("host",)                    -> "shubh@github" + rule
...
ROWS = [
    ("host",),
    ("kv", "Now", "Founding Engineer @ Nakhrali Luxury"),
    ("kv", "Edu", "B.S. Data Science, IIT Madras '27"),
    ("kv", "Also", "B.Tech CSE, UIT-RGPV Bhopal '27"),
    ("kv", "Seeking", "SDE / Backend / ML intern 2026"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Frontend", "React, Next.js, Vue, TypeScript"),
    ("kv", "Backend", "Node, Express, Flask, FastAPI"),
    ("kv", "Data", "PostgreSQL, Redis, Celery, Supabase"),
    ("kv", "AI / ML", "scikit-learn, XGBoost, MLflow, Gemini"),
    ("kv", "Cloud", "Docker, Vercel, Cloudflare R2"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "5th / 2,343 on a public ML leaderboard"),
    ("bul", "Live D2C store, 1,000+ SKUs, 84 endpoints"),
    ("bul", "Best Project Award - IIT Madras"),
    ("bul", "Co-founded communities, 5,600+ members"),
]
...
for i, row in enumerate(ROWS):
    ...
    if kind == "gap":
        ...
        continue
    if kind == "host":
        host_w = int((len(HOST_USER) + len(HOST_NAME) + 1) * 8.4) + 8
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">{HOST_USER}</tspan><tspan fill="{MUTED}">@</tspan>'
                 f'<tspan fill="{ACCENT}">{HOST_NAME}</tspan></text>'
