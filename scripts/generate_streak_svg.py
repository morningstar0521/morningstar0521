...
USER = sys.argv[1] if len(sys.argv) > 1 else "morningstar0521"
...
def get_data(user):
    ...
    try:
        ...
        # fallback: rebuild from the scraped snapshot in data/contributions.json
        here = os.path.dirname(os.path.abspath(__file__))
        snap = os.path.join(here, "..", "data", "contributions.json")
        if os.path.exists(snap):
            print("API failed (%s); using data/contributions.json" % e)
            raw = json.load(open(snap))
            days = raw["days"]
            best = max((d["count"] for d in days), default=0) or 1
            def level(c):
                if c == 0: return 0
                if c <= max(1, best * 0.25): return 1
                if c <= max(2, best * 0.50): return 2
                if c <= max(3, best * 0.75): return 3
                return 4
            return {
                "contributions": [
                    {"date": d["date"], "count": d["count"], "level": level(d["count"])}
                    for d in days
                ],
                "total": {"lastYear": raw["total_contributions"]},
            }
        raise
